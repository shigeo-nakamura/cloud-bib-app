...
class HttpUtility(private val context: Context) {
    ...
    private fun makeHttpPostRequest(urlString: String, postData: String): HttpURLConnection? {
    ...
    fun returnBook(book_id: String): HttpResponse {
        try {
            ...
            connection = makeHttpPostRequest("$urlString/work/process", postData)
    ...
    fun borrowBook(book_id: String): HttpResponse {
        try {
            ...
            connection = makeHttpPostRequest("$urlString/work/process", postData.toString())
            ...
            if (connection?.responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (jsonResponse != null) {
                    return if (jsonResponse.optBoolean("success")) {
                        ...
                        val borrowedBookObject = jsonResponse.getJSONArray("borrowed_books").getJSONObject(0)
                        var borrowedBook = BorrowedBook(borrowedBookObject.getInt("book_id"), borrowedBookObject.getString("book_title"))
